...
在发生 Minor GC 时，由于 Survivor 区已经放不下了，多出的对象只能提升（promotion）到老年代。但是此时老年代因为空间碎片的缘故，会发生 concurrent mode failure （并发模式失败）的错误。这个时候，就需要降级为 Serail Old 垃圾回收器进行收集。这就是比 concurrent mode failure 更加严重的 promotion failed （晋升失败）问题。

![](D:\study\学习资料\笔记\JVM\图片\第七讲\1.jpg)
...
​             ![](D:\study\学习资料\笔记\JVM\图片\第七讲\5.jpg)
...
![](D:\study\学习资料\笔记\JVM\图片\第七讲\6.jpg)
